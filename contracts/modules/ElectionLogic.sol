...
abstract contract ElectionLogic {
    ...
    /**
     * @notice Récupère de manière verbose le statut d'un round ainsi qu'un message explicatif.
     * @param _electionId L'identifiant de l'élection.
     * @param _round Le numéro du round.
     * @return Un tuple contenant le statut (RoundStatus) et un message explicatif (string).
     */
    function getRoundStatusVerbose(
        uint256 _electionId,
        uint8 _round
    ) public view returns (RoundStatus, string memory) {
        ElectionTypes.Round storage round = electionRounds[_electionId][_round];

        if (round.finalized) {
            return (RoundStatus.Ended, unicode"La session est finalisée.");
        }

        if (block.timestamp >= round.endDate) {
            // Même non finalisé, il a dépassé la fin → considéré comme terminé
            return (RoundStatus.Ended, unicode"La date de fin est dépassée.");
        }

        if (block.timestamp >= round.startDate) {
            return (RoundStatus.Active, unicode"La session est en cours.");
        }

        return (
            RoundStatus.NotStarted,
            unicode"La session n'a pas encore commencé."
        );
    }

    /**
     * @notice Récupère le statut d'un round.
     * @param _electionId L'identifiant de l'élection.
     * @param _round Le numéro du round.
     * @return Le statut du round (RoundStatus).
     */
        ...
        )
    {
        ...
        require(
            block.timestamp > round.endDate || round.finalized,
            "Round not over yet"
        );
    ...
    function finalizeRound(
        ...
        // Si aucun vote valide n'a été exprimé (tous votes sont blancs)
        if (validVotes == 0) {
            // On considère que le tour est finalisé, l'élection se termine
            round.finalized = true;
            e.isActive = false;
            e.winnerCandidateId = 0; // On peut définir 0 comme non attribué ou vote blanc
            emit RoundFinalized(electionId, roundNumber, true, 0);
            return;
        }
